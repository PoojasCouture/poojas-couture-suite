...
function checkAccess(isRetry) {
  ...
  // Apply default auto-delegate ON state
  const toggle = document.getElementById('delegate-toggle');
  if (toggle) toggle.classList.add('on', 'visible');
  const input = document.getElementById('user-input');
  if (input) input.placeholder = 'Describe what you need — Priya will delegate to the team automatically…';
  renderMessages();
}
