...
const Accounting = (() => {
  ...
  function render() {
    ...
    container.innerHTML = `
      ...
      <div class="animate-fade-in" style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px;">
        <button class="tab-btn tab-btn-icon ${activeTab === 'dashboard' ? 'active' : ''}" data-tab="dashboard" data-tooltip="Overview"><img src="/assets/30_bar_chart_card.png" alt="Overview"></button>
        <button class="tab-btn tab-btn-icon ${activeTab === 'invoices' ? 'active' : ''}" data-tab="invoices" data-tooltip="Invoices"><img src="/assets/02_clipboard_pencil.png" alt="Invoices"></button>
        <button class="tab-btn tab-btn-icon ${activeTab === 'expenses' ? 'active' : ''}" data-tab="expenses" data-tooltip="Expenses"><img src="/assets/18_green_coin_purse.png" alt="Expenses"></button>
        <button class="tab-btn tab-btn-icon ${activeTab === 'gst' ? 'active' : ''}" data-tab="gst" data-tooltip="GST BAS Report"><img src="/assets/33_australia_badge.png" alt="GST BAS Report"></button>
        <button class="tab-btn tab-btn-icon ${activeTab === 'reports' ? 'active' : ''}" data-tab="reports" data-tooltip="Financial Reports"><img src="/assets/27_bar_chart_purple.png" alt="Financial Reports"></button>
      </div>
  ...
  // Record payment against an invoice's next unpaid milestone
  // Looked up directly by invoiceId (unlike CRM.recordMilestonePayment, which
  // requires a projectId + a pre-known milestoneIndex from the Project view).
  function recordMilestonePayment(invoiceId) {
    ...
    const milestoneIndex = milestones.findIndex(ms => !ms.paid);
    if (milestoneIndex === -1) { Utils.showToast('All milestones on this invoice are already paid.', 'info'); return; }
      ...
      content: `
        <form id="milestone-pay-form" class="animate-fade-in-scale">
          ...
          <div class="form-group">
            ...
            <input type="text" inputmode="decimal" autocomplete="off" name="amount" class="form-input" id="acct-milestone-amount-input"
              min="0" step="0.01" value="${mBalance}" required>
            <div id="acct-milestone-pay-hint" class="text-xs text-muted mt-1">
    ...
    setTimeout(() => {
      const input = document.getElementById('acct-milestone-amount-input');
      const hint  = document.getElementById('acct-milestone-pay-hint');
