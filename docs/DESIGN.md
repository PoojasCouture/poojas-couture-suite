...
- **Touch-safe tooltips:** native `title=` tooltips never show on phones. For elements inside a scrolling/overflow container (e.g. the Shipping KPI drawer, which clips CSS `::after` tooltips), use a single fixed-position element positioned by JS on tap/hover (`showKpiTip` in `shipping/app.js`, `#kpi-tip` CSS in `shipping/index.html`). Elsewhere, use `data-tooltip` + CSS `:hover::after` like the collapsed sidebar.
